% ship corrects path using a more ideal movent. arctan movement which is
% much smoother and more ideal for ships to use. 
...
while norm(red_ship_position - goal_position)>0.4
    ...
    if distance < 8 && cir_flag ==0 && obstacle_pass ==0
         cir_flag =1;
         
         %determine the center of circle
         center = red_ship_position + (goal_position - init_position)/norm(goal_position - init_position)*3;
